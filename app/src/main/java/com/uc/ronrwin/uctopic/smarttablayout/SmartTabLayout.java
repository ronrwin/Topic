 ...
 */
package com.uc.ronrwin.uctopic.smarttablayout;
...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            int start = (w - TabUtils.getMeasuredWidth(firstTab)) / 2 - TabUtils.getMarginStart(firstTab);
            int end = (w - TabUtils.getMeasuredWidth(lastTab)) / 2 - TabUtils.getMarginEnd(lastTab);
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        final boolean isLayoutRtl = TabUtils.isLayoutRtl(this);
        View selectedTab = tabStrip.getChildAt(tabIndex);
        int widthPlusMargin = TabUtils.getWidth(selectedTab) + TabUtils.getMarginHorizontally(selectedTab);
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                int selectHalfWidth = TabUtils.getWidth(selectedTab) / 2 + TabUtils.getMarginEnd(selectedTab);
                int nextHalfWidth = TabUtils.getWidth(nextTab) / 2 + TabUtils.getMarginStart(nextTab);
            ...
            int x;
            if (isLayoutRtl) {
                int first = TabUtils.getWidth(firstTab) + TabUtils.getMarginEnd(firstTab);
                int selected = TabUtils.getWidth(selectedTab) + TabUtils.getMarginEnd(selectedTab);
                x = TabUtils.getEnd(selectedTab) - TabUtils.getMarginEnd(selectedTab) - extraOffset;
                x -= (first - selected) / 2;
            } else {
                int first = TabUtils.getWidth(firstTab) + TabUtils.getMarginStart(firstTab);
                int selected = TabUtils.getWidth(selectedTab) + TabUtils.getMarginStart(selectedTab);
                x = TabUtils.getStart(selectedTab) - TabUtils.getMarginStart(selectedTab) + extraOffset;
                x -= (first - selected) / 2;
            }
        ...
        if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
            ...
            if (isLayoutRtl) {
                x = -TabUtils.getWidthWithMargin(selectedTab) / 2 + getWidth() / 2;
                x -= TabUtils.getPaddingStart(this);
            } else {
                x = TabUtils.getWidthWithMargin(selectedTab) / 2 - getWidth() / 2;
                x += TabUtils.getPaddingStart(this);
            }
        ...
        int start = TabUtils.getStart(selectedTab);
        int startMargin = TabUtils.getMarginStart(selectedTab);
        if (isLayoutRtl) {
            x += start + startMargin - extraOffset - getWidth() + TabUtils.getPaddingHorizontally(this);
        } else {
