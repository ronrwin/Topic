...
class SmartTabStrip extends LinearLayout {
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            int selectedStart = TabUtils.getStart(selectedTab, indicatorWithoutPadding);
            int selectedEnd = TabUtils.getEnd(selectedTab, indicatorWithoutPadding);
            int left;
            int right;
            if (isLayoutRtl) {
                left = selectedEnd;
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                int nextStart = TabUtils.getStart(nextTab, indicatorWithoutPadding);
                int nextEnd = TabUtils.getEnd(nextTab, indicatorWithoutPadding);
                if (isLayoutRtl) {
...
//                Log.d("selectionOffset", "" + selectionOffset);
            }

            if (indicatorWidth == AUTO_WIDTH) {
                int length = Math.abs(left - right);
                int half = length / 2;
                int padding = length * 3 / 8;
                int spare = (int) ((half - padding) * (1 - selectionOffset));

                View firstTab = getChildAt(0);
                if (selectedPosition == 0) {
                    ((TabView) firstTab).titleIcon.setChangeAnim(selectionOffset);
//                    ((TabView) firstTab).setchangeAlpha(1f);
                }
                if (left < firstTab.getRight() / 4) {
                    if (selectionOffset < 0.001f) {
                        left = left + half;
                        right = left;
                    } else {
                        left = left + padding + spare;
                        right = right - padding - spare;
                    }
                } else {
                    left = left + padding;
                    right = right - padding;
    ...
    private void drawSeparator(Canvas canvas, int height, int tabCount) {
        ...
        for (int i = 0; i < tabCount - 1; i++) {
            ...
            int end = TabUtils.getEnd(child);
            int endMargin = TabUtils.getMarginEnd(child);
...
//            int length = Math.abs(left - right);
//            int half = length / 2;
//            int padding = length * 2 / 5;
//
//            if (selectedPosition == 0) {
//                left = (int)(left + (half - padding) * selectionOffset);
//                right = (int)(right - (half - padding) * selectionOffset);
//            }
//
